...
import { getSingleFLogArrP4, getSingleFLogArrP2 } from "./Logs/log.utils";
import { styles } from "../../styles/styles";
import CheckBox from "../CheckBox";
import { Buffer } from "buffer";
const transactionId = "monitor_battery";

class BLEFaultLogWidget extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showLoader: false,
  ...
  generateData() {
    const { logData, isShowAllPackets } = this.state;
    let logValuesData = [];
    if (logData.length === 0) {
      let newLogData = [];
      for (let i = 0; i < this.state.totalLogToDisplay; i++) {
        newLogData.push(
          isShowAllPackets ? getSingleFLogArrP4() : getSingleFLogArrP2()
        );
        logValuesData.push([]);
      }
  ...
  handleLoopOfLogsResponses(junkRemovedStr) {
    if (
      ...
      console.log("totalLogToDisplay in handleloop 111 = ", totalLogToDisplay);
      if (currentLogIndex < totalLogToDisplay - 1) {
        ...
        console.log(
          "totalLogToDisplay in handleloop 2222 = ",
          this.state.totalLogToDisplay
        );
        this.setState(
          {
            logValuesData: this.getLogValuesDataSlot(junkRemovedStr),
        ...
        this.setState({ lock: true, showLoader: true }, () => {
          if (isConnected === false) {
            ...
            obj.logEntries.forEach(element => {
              ...
              addToLocalStorage(element, id, "", "", isShowAllPackets).then(
                response => {
                  count++;
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => Keyboard.dismiss()}>
          <View>
            {getTopContainerIcons(
              ...
              () =>
                sendMail(
                  ...
                  "FAULTLOG", // ErrorFaultLogStr string
            ...
            <View style={{ marginTop: "2%" }}>
              <CheckBox
                isCheck={this.state.isShowAllPackets}
                onChangeCheckBox={() => {
                  this.setState({
                    isShowAllPackets: !this.state.isShowAllPackets
                  });
                }}
                title="Enable extended data"
              />
            </View>
          </View>
          {getTextFieldAndResultbutton(
