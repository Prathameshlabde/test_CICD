...
import { getJsonValuesArrForTimers, getJsonObjArr } from "./Logs/log.utils";
const transactionId = "monitor_battery";

class BLEWidgetCopy extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showLoader: false,
  ...
  getFinalJsonArr(jsonFile, id) {
    ...
    if (isBasicScreen) {
      return getNewJsonWithValue(jsonFile, valuesArr);
    } else if (isTimerScreen) {
      const newJsonArr = getJsonObjArr(id);
      return getJsonValuesArrForTimers(newJsonArr, valuesArr);
    }
  }

  render() {
    const { showLoader, deviceID, peripheral, isAllDataReceived } = this.state;
    const { jsonFile, id } = this.props;
    let finalJsonArr;
    if (isAllDataReceived) finalJsonArr = this.getFinalJsonArr(jsonFile, id);
    else finalJsonArr = [];
    return (
